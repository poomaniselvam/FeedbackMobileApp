...
export default function App() {
  ...
  const [backgroundColor, setBackgroundColor] = useState("red"); // Default color
  ...
  useEffect(() => {
    axios.get('https://mediumblue-jellyfish-250677.hostingersite.com/api/question')
     ...
     axios.get('https://mediumblue-jellyfish-250677.hostingersite.com/api/navbar')
      .then((response) => {
        const navbars = response.data;
        if (Array.isArray(navbars) && navbars.length > 0) {
          const lastNavbar = navbars[navbars.length - 1];
          if (lastNavbar && lastNavbar.background_color) {
            setBackgroundColor(lastNavbar.background_color);
          }
        }
      })
      .catch((error) => {
